...
// Force pure‑JS CPU backend for TensorFlow.js
// Suppress TF.js banner for cleaner logs
...
exports.faceMatch = async (req, res) => {
  try {
    ...
    for (const stu of students) {
      if (!Array.isArray(stu.faceDescriptor) || stu.faceDescriptor.length !== 128) continue;
      const dist = faceapi.euclideanDistance(inputDescriptor, stu.faceDescriptor);
      if (dist < minDistance) {
        minDistance = dist;
        bestMatch   = stu;
      }
